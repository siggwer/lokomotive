...
func (c *config) terraformCreateReservations(ex *terraform.Executor) error {
	...
	if len(c.ReservationIDs) > 0 {
		arguments = append(arguments, fmt.Sprintf("-target=module.packet-%v.packet_device.controllers", c.ClusterName))
	}
...
func (c *config) checkValidConfig() hcl.Diagnostics {
	// TODO: this function really needs to be tested
...
func checkEachReservation(reservationIDs map[string]string, resDefault, nodeRole, name string) hcl.Diagnostics {
	...
	// Avoid cases that set (to non default values) reservation_ids and reservation_ids_default
	if len(reservationIDs) > 0 && resDefault != "" {
		diagnostics = append(diagnostics, &hcl.Diagnostic{
			...
			Detail:   fmt.Sprintf("%v: %v sets both, instead add an entry in reservations_ids for each node", errorPrefix, name),
		})
	}
	...
	for _, v := range reservationIDs {
		...
		diagnostics = append(diagnostics, &hcl.Diagnostic{
			...
			Detail:   fmt.Sprintf("%v: %v uses it, use specific UUIDs or reservations_ids_default only", errorPrefix, name),
		})
	}

	// Check format is:
	// controller-<int> or worker-<int>
	// If not, terraform code will just ignore it
	resPrefix := "worker-"
	if nodeRole == "controller" {
...
// checkResFormat checks that format for every key in reservationIDs is: <resPrefix>-<int>
func checkResFormat(reservationIDs map[string]string, name, errorPrefix, resPrefix string) hcl.Diagnostics {
	...
	for key := range reservationIDs {
		hclErr := &hcl.Diagnostic{
			Severity: hcl.DiagError,
			Summary:  "Format used in reservation_ids invalid",
			Detail: fmt.Sprintf("%v: %v used \"%v\", format should be \"%v<int>\"",
		...
		// Check a valid number is used after "controller-" or "worker-
		index := resEntry[1]
